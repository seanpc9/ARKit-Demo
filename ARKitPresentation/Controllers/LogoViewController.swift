...
class LogoViewController: BaseSceneViewController {
    ...
    private func addBear(with hitTestResult: ARHitTestResult) {
        ...
        let logoScene = SCNScene(named: "art.scnassets/New_VT_logo.scn")!
        
        // Grab the actual bear object from the 3D model (ignoring other objects in the model)
        let logoNode = logoScene.rootNode.childNode(withName: "New_VT_logo", recursively: true)!
        
        // Scale down the bear since it's too large
        logoNode.scale = SCNVector3(0.0008, 0.0008, 0.0008)
        
        // Add physics to the bear
        logoNode.physicsBody = SCNPhysicsBody(type: .dynamic, shape: nil)
        logoNode.physicsBody?.categoryBitMask = BodyType.logo.rawValue
        logoNode.physicsBody?.mass = 0.1
        
        // Position the bear relative to the hit test point
        let hitTestWorldTransform = hitTestResult.worldTransform.columns.3
        logoNode.position = SCNVector3(hitTestWorldTransform.x,
